...
@router.callback_query(builders.PaginationButtonCallback.filter(F.query.in_(["previous_pairs", "next_pairs"])))
async def paginate_pairs(callback_query: CallbackQuery, callback_data: builders.PaginationButtonCallback):
  ...
  await callback_query.message.edit_reply_markup(
    reply_markup=await builders.render_pairs_kb(callback_query.from_user.id, str(page))
  )
  
@router.callback_query(inline.PaginationButtonCallback.filter(F.query == "back_to_pairs"))
async def back_to_pairs(callback_query: CallbackQuery, callback_data: inline.PaginationButtonCallback):
...
@router.callback_query(builders.PaginationButtonCallback.filter(F.query.in_(["view_pair", "back_to_pair"])))
async def view_pair(callback_query: CallbackQuery, callback_data: builders.PaginationButtonCallback):
