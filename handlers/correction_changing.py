...
@router.callback_query(inline.PaginationButtonCallback.filter(F.query == "change_correction_rate"))
async def change_correction_rate(callback_query: CallbackQuery, callback_data: inline.PaginationButtonCallback, state: FSMContext): 
  ...
  await callback_query.message.edit_text(
    text="ОК. Введите новое значение параметра."
  )
...
async def check_correction_rate(message: Message, state: FSMContext):
  if await filters.is_float(message.text):
    ...
    await message.answer("Пожалуйста, введите целое число или число, с плавающей точкой.")