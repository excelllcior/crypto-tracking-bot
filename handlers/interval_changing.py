...
@router.callback_query(
    inline.PaginationButtonCallback.filter(F.query == "change_interval")
)
...
@router.callback_query(
    inline.PaginationButtonCallback.filter(F.query == "interval_selected")
)
async def complete_interval_changing(
    callback_query: CallbackQuery, callback_data: inline.PaginationButtonCallback
):
...
@router.callback_query(
    builders.PaginationButtonCallback.filter(
        F.query.in_(["previous_intervals", "next_intervals"])
    )
)
async def paginate_pairs(
    ...
    await callback_query.message.edit_reply_markup(
        reply_markup=await builders.render_intervals_kb(
            pair_id, pairs_page, str(intervals_page)
        )
    )
