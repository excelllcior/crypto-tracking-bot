...
async def check_pair(pair_id: str) -> None:
  ...
  if pair is not None:
    ...
    if await filters.is_on_trading_view(pair_name, exchange_name):
      ...
      print(f"INFO:monitoring:Пара {pair_id} была удалена из списка отслеживаемых")
    ...
    tracking_pairs_ids.remove(pair_id)
    print(f"INFO:monitoring:Пользователь удалили пару {pair_id} из списка отслеживаемых")
...
async def check_macd(pair: Row) -> None:
  ...
  if (macd_line > signal_line):
    print(f"INFO:monitoring:MACD ({macd_line}) уже больше сигнальной линии ({signal_line}). Delay: {delay}. Pair ID: {pair['id']}")
...
async def macd_equal_to_signal(pair: Row, handler: TA_Handler, delay: int) -> None:
  print(f"INFO:monitoring:MACD равен сигнальной линии. Delay: {delay}. Pair ID: {pair['id']}")
  await asyncio.sleep(delay)
  analysis = handler.get_analysis()
  macd_line = analysis.indicators['MACD.macd']
  signal_line = analysis.indicators['MACD.signal']
  if (macd_line > signal_line):
    print(f"INFO:monitoring:MACD ({macd_line}) стал больше сигнальной линии ({signal_line}). Delay: {delay}. Pair ID: {pair['id']}")
...
async def macd_less_than_signal(pair: Row, handler: TA_Handler, delay: int) -> None:
  print(f"INFO:monitoring:MACD меньше сигнальной линии. Delay: {delay}. Pair ID: {pair['id']}")
  await asyncio.sleep(delay)
  analysis = handler.get_analysis()
  macd_line = analysis.indicators['MACD.macd']
  signal_line = analysis.indicators['MACD.signal']
...
async def track_growth(pair: Row, handler: TA_Handler, delay: int) -> None:
  ...
  if actual_growth_rate >= growth_rate:
    print(f"INFO:monitoring:Процент роста ({actual_growth_rate}) больше или равен заданному параметру ({growth_rate}). Delay: {delay}. Pair ID: {pair['id']}")
    await track_correction(pair, handler, delay)
  else:
    print(f"INFO:monitoring:Процент роста ({actual_growth_rate}) меньше заданного параметра ({growth_rate}). Delay: {delay}. Pair ID: {pair['id']}")
    await check_pair(pair['id'])

async def check_correction(pair: Row, handler: TA_Handler, opening: float, delay: int) -> None:
  await asyncio.sleep(delay)
  closing = handler.get_analysis().indicators['open']
  ...
  if current_correcion_rate <= -pair['correcion_rate']:
    print(f"INFO:monitoring:Пара {pair['pair_name']} c {pair['exchange_name']} достигла необходимого процента роста и коррекции. Delay: {delay}. Pair ID: {pair['id']}")
    await bot.send_message(
      chat_id=pair['chat_id'],
