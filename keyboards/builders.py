...
async def render_pairs_kb(chat_id: str, page: str) -> InlineKeyboardMarkup:
  ...
  if pairs_total > 0:
    for i in range(offset, min(offset + PER_PAGE, pairs_total)):
      builder.row(
        InlineKeyboardButton(
          ...
          callback_data=PaginationButtonCallback(query="view_pair", params=str(pairs[i]['id']), page=page).pack()
        )
      )
    ...
    builder.row(
      InlineKeyboardButton(
        text="Пред.", 
        callback_data=PaginationButtonCallback(query=previous_query, params=None, page=page).pack()
      ), 
      InlineKeyboardButton(
      ...
      InlineKeyboardButton(
        text="След.", 
        callback_data=PaginationButtonCallback(query=next_query, params=None, page=page).pack()
      ),
      width=3
    )
  else:
...
async def render_intervals_kb(pair_id: str, pairs_page: str, intervals_page: str) -> InlineKeyboardMarkup:
  PER_PAGE = 5
  builder = InlineKeyboardBuilder()
  ...
  for i in range(offset, min(offset + PER_PAGE, intervals_total)):
    builder.row(
      InlineKeyboardButton(
        ...
        callback_data=PaginationButtonCallback(query="interval_selected", params=f"{pair_id}/{intervals[i]['id']}", page=pairs_page).pack()
      )
    )  
  if offset > 0:
  ...
  builder.row(
    InlineKeyboardButton(
      text="Пред.", 
      callback_data=PaginationButtonCallback(query=previous_query, params=pair_id, page=f"{pairs_page}/{intervals_page}").pack()
    ), 
    InlineKeyboardButton(
    ...
    InlineKeyboardButton(
      text="След.", 
      callback_data=PaginationButtonCallback(query=next_query, params=pair_id, page=f"{pairs_page}/{intervals_page}").pack()
    ),
    width=3
  )
  ...
  builder.row(
    InlineKeyboardButton(
      text="« Назад", 
      callback_data=PaginationButtonCallback(query="back_to_params_kb", params=pair_id, page=pairs_page).pack()
    )
  )    
